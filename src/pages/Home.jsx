  ...
  useEffect(() => {
    let arr = games.filter((game) => {
      if (game.genre === filterBy.genre || filterBy.genre === "") {
        ...
        if (
          filterBy.platform === "" ||
          game.platform.includes(filterBy.platform)
        ) {
    ...
    if (
      (filterBy.sortBy === "most recent") |
      (filterBy.sortBy === "least recent")
    ) {
      arr.sort((a, b) => {
        let date1 = a.release_date.split("-");
        let date2 = b.release_date.split("-");
        if (parseInt(date1[0]) - parseInt(date2[0]) !== 0)
          return parseInt(date1[0]) - parseInt(date2[0]);
        if (parseInt(date1[1]) - parseInt(date2[1]) !== 0)
        ...
        <section className="filter-bar">
          ...
          <div className="c-form-select flex-item">
            ...
            >
              ...
              <option key="0" value="PC (Windows)">
                PC (Windows)
              </option>
              <option key="1" value="Web Browser">
                Web Browser
              </option>
            </select>
          </div>
          <div className="c-form-select flex-item">
            ...
            >
              ...
              <option key="1" value="most recent">
                Most Recent
              </option>
              <option key="2" value="least recent">
                Least Recent
              </option>
              <option key="3" value="alphabetically">
                alphabetically
              </option>
            </select>
          </div>
